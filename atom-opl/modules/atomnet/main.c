...
static void HeartbeatThread(void *arg)
{
    ...
    while (1) {
        if (g_device_ready) {
            /*
             * Important: do not run SET_CONFIGURATION/vendor requests from the
             * USBD connect callback. OPL's mass-storage driver also defers
             * configuration work until after connect returns. Doing the FTDI
             * transactions here keeps us outside the enumeration callback and
             * works regardless of which PS2 root port the FTDI is attached to.
             */
            if (!g_transport_configured) {
                ...
                if (result < 0) {
                    printf("ATOMNET: deferred SET_CONFIGURATION failed %d; retrying\n",
                           result);
                    DelayThread(500000);
                    continue;
                }
                ...
                if (result < 0) {
                    printf("ATOMNET: deferred FTDI setup failed %d; retrying\n",
                           result);
                    DelayThread(500000);
                    continue;
                }
                ...
                printf("ATOMNET: FTDI transport configured, heartbeat armed\n");
            }
...
static int atom_probe(int devId)
{
    ...
    if (device->idVendor != VID_FTDI ||
        (device->idProduct != 0x6001 && device->idProduct != 0x6015))
        return 0;
...
static int atom_connect(int devId)
{
    ...
    /*
     * Do not perform control/bulk transactions here. Just claim the device.
     * The heartbeat thread performs setup after this USBD callback returns.
     */
    g_device_ready = 1;

    printf("ATOMNET: FTDI attached %04X:%04X dev=%d, deferred setup queued\n",
...
int _start(int argc, char *argv[])
{
    ...
    printf("ATOMNET v0.5 DeferredFTDI: starting\n");
